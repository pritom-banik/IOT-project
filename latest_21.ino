...
void loop(){
  ...
  if(!SYSTEM_ACTIVE){
    digitalWrite(MOTOR_PIN, LOW);  // Ensuring motor is OFF
  ...
  // If system is busy processing, dont check for new objects
  if (distance > 0 && distance < baselineDistance){
    ...
    while(millis() - checkStartTime < 2000){ // Check for 2 seconds
        int proximityState = digitalRead(PROXIMITY_PIN);
        
        if(proximityState == LOW){
            metalDetected = true;
            break; // Exit loop early if metal is detected
        }
  ...
  // int proximityState2 = digitalRead(PROXIMITY_PIN);

  // if (proximityState2 == LOW) {
  //   // Metal detected
